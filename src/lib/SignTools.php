...
class SignTools extends IM
{
     ...
     * @throws \Exception
     */
    public function genSignWithUserbuf($identifier, $userbuf, $expire = 15552000)
    {
        $json = Array(
            'TLS.account_type' => '0',
            'TLS.identifier' => (string)$identifier,
            'TLS.appid_at_3rd' => '0',
            'TLS.sdk_appid' => (string)$this->config['SDKAppid'],
    ...
    public function verifySigWithUserbuf($sig, $identifier, &$init_time, &$expire_time, &$userbuf, &$error_msg)
    {
        try {
            ...
            if ($json['TLS.sdk_appid'] != $this->config['SDKAppid']) {
                throw new \Exception("appid error sigappid:{$json['TLS.appid']} thisappid:{" . $this->config['SDKAppid'] . "}");
            }
            $content = $this->genSignnContentWithUserbuf($json);
     ...
     * @return string 生成的UserSig 失败时为false
     */
    public function genSign($identifier, $expire = 15552000)
    {
        $json = Array(
            'TLS.account_type' => '0',
            'TLS.identifier' => (string)$identifier,
        ...
        $err = '';
        $content = $this->genSignnContent($json, $err);
    ...
    public function verifySig($sig, $identifier, &$init_time, &$expire_time, &$error_msg)
    {
        try {
            ...
            $content = $this->genSignnContent($json);
            $signature = base64_decode($json['TLS.sig']);
            if ($signature == false) {
                throw new \Exception('sig json_decode error');
            }
